...
function refreshLabels() {
    $('[textcontent]').each(function() {
        ...
        if (textname !== undefined && textname !== null && textname !== '') {
            $(this).text(multilang.get(textname));
        }
    });
}
...
function loadingScreen() {
    const loadingDiv = $('<div>', {
        'class': 'h-full w-full flex items-center justify-center bg-bluey-500 dark:bg-bluey-700'
    });
    const LogoDiv = $('<div>', {
        'class': 'text-center',
    });
...
function headerElement() {
    ...
    // Expand Button
    /*header.find('#nav-content').append(
        $('<div>', {
            'class': 'absolute left-0 items-center pl-2',
            'html': '<button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" class="inline-flex items-center p-2 text-sm text-white rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none hover:text-gray-900 focus:ring-2 focus:ring-gray-200"><span class="sr-only">Open sidebar</span><svg class="w-6 h-6" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path clip-rule="evenodd" fill-rule="evenodd" d="M2 4.75A.75.75 0 012.75 4h14.5a.75.75 0 010 1.5H2.75A.75.75 0 012 4.75zm0 10.5a.75.75 0 01.75-.75h7.5a.75.75 0 010 1.5h-7.5a.75.75 0 01-.75-.75zM2 10a.75.75 0 01.75-.75h14.5a.75.75 0 010 1.5H2.75A.75.75 0 012 10z"></path></svg></button>'
        })
    );*/
    ...
    themeToggleBtn.append(moon, sun);
    navContent.append(themeToggle.append(themeToggleContent.append(themeToggleBtn)));
...
function setupNetworkSection(main) {
    ...
    buttonOptions.forEach((buttonOption, index) => {
        ...
        button.click(function () {
            sectionTransition(main, sectionInstance, buttonOption.goto);
        });
...
function setupWifiSection(main) {
    ...
    const container = $('<div>', {'class': 'CONTAINER h-72 max-h-72 w-full text-gray-900 rounded-lg dark:text-white space-y-2'})
...
$(document).ready(function() {
    ...
    const main = $('<main>', {'class': 'h-full mt-5 px-4 sm:px-8 text-gray-900 dark:text-white flex justify-center flex-col',});
    app.append(main);
