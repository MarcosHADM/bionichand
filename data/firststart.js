...
class SectionClass {
    ...
    init() {
        ...
        this.backButton.append($('<span>', {'html': 'Back', 'textcontent':'Back'}));
        ...
        this.nextButton.append($('<span>', {'html': 'Next', 'textcontent': 'Next'}));
        ...
        this.sectionTitleText = $('<p>', {'class': 'section-title-text text-xl font-bold', 'html': this.TitleText, 'textcontent': this.TitleText});
        this.sectionTitle.append(this.TitleIcon, this.sectionTitleText);

        this.section.append(this.sectionHeader.append(this.backButton, this.nextButton, this.sectionTitle));
...
function selectLanguageSection(main) {
    ...
    languages.forEach((language, index) => {
        ...
        const button = $('<button>', {'language': language.code, 'type': 'button', 'class': 'my-1 relative inline-flex items-center w-full px-4 py-2 text-lg font-medium text-stone-900 dark:text-white rounded-lg hover:bg-bluey-500 dark:hover:bg-bluey-700 hover:text-white focus:z-10 focus:ring-5 focus:ring-blue-700', 'html': `${language.name}<svg class="w-3 h-3 ml-auto" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.1584 3.13514C6.35985 2.94621 6.67627 2.95642 6.86514 3.15788L10.6151 7.15788C10.7954 7.3502 10.7954 7.64949 10.6151 7.84182L6.86514 11.8418C6.67627 12.0433 6.35985 12.0535 6.1584 11.8646C5.95694 11.6757 5.94673 11.3593 6.1356 11.1579L9.565 7.49985L6.1356 3.84182C5.94673 3.64036 5.95694 3.32394 6.1584 3.13508Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>`});
        button.click(() => {
            setup.Language = language.code;
...
function setupNetworkSection(main) {
    ...
    const section = sectionInstance.section;
    sectionInstance.nextButton.css('visibility', 'hidden');
    sectionInstance.backButton.click(() => sectionTransition(main, sectionInstance, selectLanguageSection));
    ...
    buttonOptions.forEach((buttonOption, index) => {
        ...
        button.append(buttonText).click(() => sectionTransition(main, sectionInstance, buttonOption.goto));
...
function updateWiFi(networksDiv) {
    ...
    const searchingNetworks = $('<p>', {'html': 'Searching for WiFi networks...', 'textcontent': 'Searching for WiFi networks...', 'class': 'font-bold text-xl text-center text-gray-400 dark:text-stone-700 mb-20'});
    const noNetworks = $('<p>', {'html': 'No networks found.', 'textcontent': 'No networks found.', 'class': 'font-bold text-xl text-center text-gray-400 dark:text-stone-700 mb-20'});
    const networks = $('<div>', {'class': 'h-full w-full overflow-y-auto', 'style': 'display: none;'});
    
    networksDiv.empty().append(searchingNetworks.fadeIn('fast'));
    
    searchingNetworks.promise().done(() => {
        $.ajax({
            type: 'GET',
            url: '/wifi/scan',
            dataType: 'json',
                ...
                searchingNetworks.fadeOut('fast', () => {
                    ...
                    if ($.isEmptyObject(response)) {
                        ...
                        json.forEach(network => {
                            ...
                            const networkButtonDiv = $('<div>', {'class': 'inline-flex items-start w-full'}).appendTo(networkButton);
                            
                            const networkButtonDivText = $('<div>', {'class': 'w-full text-ellipsis overflow-hidden whitespace-nowrap text-left'}).appendTo(networkButtonDiv);
                            const networkButtonDivTextSSID = $('<p>', {'html': network.ssid, 'class': 'text-ellipsis overflow-hidden whitespace-nowrap'}).appendTo(networkButtonDivText);
                            ...
                            networkButton.click(() => {
                                ...
                                const passwordLabel = $('<label>', {'for': 'password', 'html': 'Enter network security key', 'textcontent': 'Enter network security key', 'class': 'text-left block mb-1 text-sm font-medium text-gray-900 dark:text-white'}).appendTo(passwordDiv);
                                const password = $('<input>', {'type': 'password', 'name': 'password', 'placeholder': '••••••••', 'class': 'bg-gray-200 dark:bg-stone-900 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'}).appendTo(passwordDiv);
                                const passwordMinChars = $('<p>', {'html': 'Password must be at least 8 characters long', 'textcontent': 'Password must be at least 8 characters long', 'class': 'text-sm text-red-500 font-semibold', 'style': 'visibility: hidden;'}).appendTo(passwordDiv);
                                const passwordError = $('<p>', {'html': 'Error connecting to the network', 'textcontent': 'Error connecting to the network', 'class': 'text-sm text-red-500 font-semibold', 'style': 'visibility: hidden;'}).appendTo(passwordDiv);

                                if (isSecure) modal.content.append(passwordDiv);
                    
                                const connectButton = $('<button>', {'type': 'button', 'html': 'Connect', 'textcontent': 'Connect', 'class': 'connect inline-flex w-full justify-center rounded-md bg-bluey-500 dark:bg-bluey-700 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-bluey-600 hover:dark:bg-bluey-600 sm:ml-3 sm:w-auto'});
                                const cancelButton = $('<button>', {'type': 'button', 'html': 'Cancel', 'textcontent': 'Cancel', 'class': 'cancel mt-3 inline-flex w-full justify-center rounded-md bg-gray-200 dark:bg-stone-950 hover:bg-gray-300 hover:dark:bg-stone-900 px-3 py-2 text-sm font-semibold shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto'});
                                modal.buttons.append(connectButton, cancelButton);
                    
                                networksDiv.append(modal.modal.fadeIn("fast"));
                                ...
                                modal.modal.on('mousedown', function (e) {
                                    if (isTryingToConnect) return;
                                    if (!modal.container.is(e.target) && modal.container.has(e.target).length === 0) modal.modal.fadeOut("fast", () => {modal.modal.remove();});
                                });

                                modal.modal.on('keydown', function (e) {if (e.key === 'Escape' || e.keyCode === 27) modal.modal.mousedown()});
                                password.on('keydown', function (e) {if (e.key === 'Enter' || e.keyCode === 13) connectButton.click()});

                                connectButton.click(function () {
                                    if (isTryingToConnect) return;
                                    passwordMinChars.css('visibility', 'hidden');
                                    passwordError.css('visibility', 'hidden');

                                    if (isSecure) {
                                        if (password.val().length < 8) {
                                            passwordError.before(passwordMinChars.css('visibility', 'visible'));
                                            password.focus().keydown(() => {
                                                if (password.val().length >= 7) passwordMinChars.css('visibility', 'hidden');
                                            });
                                            return;
                                        }
                                    } else password.val("");

                                    passwordMinChars.before(passwordError);

                                    isTryingToConnect = true;
                                    modal.containerInner.css('filter', 'blur(2px)');
                                    modal.buttons.css('filter', 'blur(2px)');

                                    const loadingContainter = $('<div>', {'class': 'fixed h-full w-full top-0 right-0 flex justify-center items-center z-20'})
                                        .css('filter', 'blur(-2px)')
                                        .append($('<svg class="animate-spin h-12 w-12 text-bluey-500 dark:text-bluey-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>'));
                                    modal.modal.append(loadingContainter);

                                    setup.SSID = network.ssid,
                                    setup.Password = password.val(),

                                    $.ajax({
                                        type: "POST",
                                        url: "/wifi/connect",
                                        contentType: "application/json",
                                        data: JSON.stringify({ssid: network.ssid, password: password.val()}),
                                        success: function (response) {
                                            console.log(response);
                                            /*setup.ConnectionMethod = "WiFi";
                                            setup.SSID = network.ssid;
                                            setup.Password = password.val();

                                            const connectedModal = new ModalClass('wifi-result', 'Successfully connected');
                                            connectedModal.init();
                                            connectedModal.icon.addClass('p-2 bg-green-100 text-green-600').append($('<svg class="" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="oc se axy"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"></path></svg>'));
                                            connectedModal.content.append($('<p>', { 'class': 'text-md', 'html': 'You are now connected to the internet network; we will redirect you to the page where you can control your <strong>handmotion</strong> device.' }));
                                            const okButton = $('<button>', { 'type': 'button', 'class': 'inline-flex w-full justify-center rounded-md bg-bluey-500 dark:bg-bluey-700 px-3 py-2 text-md font-semibold text-white shadow-sm hover:bg-bluey-600 hover:dark:bg-bluey-600', 'html': 'Ok' }).appendTo(connectedModal.buttons);
                                            
                                            modal.modal.fadeOut('fast', () => {
                                                networksDiv.append(connectedModal.modal.fadeIn('fast'));
                                                modal.modal.remove();
                                            });
...
function setupWifiSection(main) {
    ...
    const section = sectionInstance.section;
    sectionInstance.nextButton.css('visibility', 'hidden');
    sectionInstance.backButton.click(() => sectionTransition(main, sectionInstance, setupNetworkSection));
    ...
    container.append(updateButton.append(updateButtonLogo, updateButtonText));
    sectionInstance.sectionContent.append(content.append(container));

    updateWiFi(networksDiv);
    updateButton.click(() => updateWiFi(networksDiv));
    return section;
}
...
function setupApSection(main) {
    ...
    const ssidLabel = $('<label>', {'for': 'SSID', 'html': 'SSID', 'class': 'block mb-1 text-sm font-medium text-gray-900 dark:text-white' });
    ...
    formContent.append(ssidDiv.append(ssidLabel, ssid));

    const passwordDiv = $('<div>', {'class': 'mt-2'});
    const passwordLabel = $('<label>', {'for': 'PASSWORD','html': 'Password', 'textcontent': 'Password', 'class': 'block mb-1 text-sm font-medium text-gray-900 dark:text-white'});
    const password = $('<input>', {'type': 'password','name': 'PASSWORD','placeholder': '••••••••','class': 'bg-gray-200 dark:bg-stone-900 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500','autocomplete': false});
    formContent.append(passwordDiv.append(passwordLabel, password));

    const eigthChars = $('<p>', {'html': 'Password must be at least 8 characters long', 'textcontent': 'Password must be at least 8 characters long', 'class': 'text-sm text-red-500 font-semibold', 'style': 'visibility: hidden;'});
    ...
    const setupButtonText = $('<p>', {'html': 'Setup', 'textcontent': 'Setup', 'class': 'text-lg font-medium text-stone-900 dark:text-white hover:text-white'});
    formContent.append(setupButtonDiv.append(setupButton.append(setupButtonText)));
    ...
    setupButton.click(function() {
        ...
        if (password.val().length < 8) {
            ...
            setup.ConnectionMethod = "AP";
            setup.SSID = ssid.val();
            setup.Password = 'HANDMOTION-DEVICE';
